...
本 repository 根目錄即 **`fng_trading/`**（專案根目錄）。
...
│   └── test_data/            # 樣本資料與回測輸出（gitignore，執行時自動建立）
├── trade/                    # 實盤
...
- `evaluate_latest_signal` — 實盤：最新兩根 **UTC 00:00 日線**（昨日 vs 前日；略過盤中 `now`）
- `evaluate_signal_on_date` — 測試：指定 `YYYY-MM-DD` 的日線 vs 前一個日線
- 回傳 `DailySignal` dataclass（含 `should_trade`、理論 TP/SL 價等）  

### `backtest/backtest_io.py`

- `ensure_test_data_dir` — 自動建立 `backtest/test_data/`（寫入前呼叫，行為類似 `trade/runtime_io.ensure_runtime_dir`）

### `backtest/fear_greed_chart_fetch.py`

函式 `fetch_fear_greed_chart_to_file(start, end, output, convert_id)`：依**呼叫端傳入**的參數抓取並寫入 JSON。獨立 CLI 的預設區間來自 `fng_backtest_with_binance_tp`。

### `backtest/fng_backtest_with_binance_tp.py`

依 `--start` / `--end`（UTC，`YYYY-MM-DD HH:MM:SS`）**自動先抓**對應區間恐慌指數（start 前多抓 2 天供 `prev_score`），再回測（僅 **UTC 00:00 日線**）→ 拉 Binance K 線 → `apply_trade_logic` → 輸出 JSON 與圖表。`--skip-fetch` 可略過 CMC、只用本地 JSON。
...
載入專案根目錄 **`.env`**（`bn_api_key`、`bn_api_secret` 等）。  
若安裝 `python-dotenv` 會優先使用；否則改用手動解析。
...
# 回測（會依期間自動抓恐慌指數，再跑回測）
python3 -m fng_trading.backtest.fng_backtest_with_binance_tp \
  --start "2024-01-07 00:00:00" \
  --end "2026-05-30 23:59:59"

# 僅用本地 JSON、不呼叫 CMC
python3 -m fng_trading.backtest.fng_backtest_with_binance_tp --skip-fetch

# 單獨更新恐慌指數 JSON（可選）
python3 -m fng_trading.backtest.fear_greed_chart_fetch \
  --start "2024-01-07 00:00:00" --end "2026-05-30 23:59:59"
```
...
建議放在專案根目錄 **`.env`**（`/path/to/fng_trading/.env`，勿提交 git）。
...
所有指令請在**專案根目錄** `fng_trading/` 下執行，並設定 `PYTHONPATH`（見 [專案結構](#專案結構)）。
...
```cron
TZ=UTC
0 0 * * * sleep 30 && /bin/bash /path/to/fng_trading/trade/run_fng_daily_cron.sh
```

腳本會自行寫入 `trade/runtime/fng_cron.log`，crontab 通常**不必**再 `>>` 導向。

### 使用包裝腳本

```bash
cd /path/to/fng_trading
bash trade/run_fng_daily_cron.sh
```

`run_fng_daily_cron.sh` 會：

- 將專案根目錄的**父目錄**加入 `PYTHONPATH` 並執行 `python -m fng_trading.trade.fng_daily_trader`  
- 載入專案根目錄 **`.env`**  
- 設定 `FNG_DRY_RUN`、`FNG_ORDER_QTY` 等（**請依需求改腳本內 export**）  
- 優先使用專案內 `.venv/bin/python3`  
...
依賴定義見專案根目錄 [`requirements.txt`](requirements.txt)：

| 套件 | 用途 |
|------|------|
| `pandas` | 恐慌指數、K 線、回測 DataFrame |
| `requests` | CMC / Binance HTTP API |
| `matplotlib` | 回測權益曲線等圖表 |
| `python-dotenv` | 可選；載入 `.env`（未安裝時改用手動解析） |
...
僅跑實盤、不需回測圖表時可省略 matplotlib：

```bash
pip install "pandas>=2.2,<4" "requests>=2.31,<3" "python-dotenv>=1.0,<2"
```
...
確認專案根目錄 `.env` 內有 `bn_api_key` / `bn_api_secret`。
